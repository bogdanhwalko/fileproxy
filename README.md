...
Після першого `/start` бот попросить поділитися контактом через Telegram-кнопку, візьме номер із контакту і згенерує 6-значний код для цього номера.
Надалі номер запам'ятовується за Telegram-користувачем, тому кожна наступна команда `/start` одразу оновлює код без повторного запиту контакту.
