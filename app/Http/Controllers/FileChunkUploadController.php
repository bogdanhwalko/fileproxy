...
class FileChunkUploadController extends Controller
{
    ...
    public function uploadChunk(Request $request, ManagedFileStorageService $fileStorage): JsonResponse
    {
        ...
        $tagIds = $this->resolveTagIdsFromMeta($meta['tags'] ?? null, $user->id);
     ...
     */
    private function resolveTagIdsFromMeta(?string $rawTags, int $userId): array
    {
        $names = Tag::parseInput($rawTags);
        if ($names === []) {
            return [];
        }
        ...
        foreach ($names as $name) {
            $tag = Tag::firstOrCreate(['user_id' => $userId, 'name' => $name]);
            $ids[] = $tag->id;
        }
