@auth
<div class="fp-palette" data-palette hidden>
...
(() => {
    ...
    const SEARCH_URL = @json(route('palette.search'));
    const FILES_URL  = @json(route('files.index'));
    const STATS_URL  = @json(route('stats.index'));
    const TG_URL     = @json(route('telegram-settings.index'));
